...
const iOS = 'Mozilla/5.0 (iPhone; CPU iPhone OS 8_0 like Mac OS X) AppleWebKit/538.34.9 (KHTML, like Gecko) Mobile/12A4265u'
...
class Platform extends Component {
  ...
  onClick = (userAgent) => {
    if (this.state.userAgent !== userAgent) {
      if (userAgent === iOS) {
        this.setState({userAgent: userAgent, ios: true, android: false})
      } else {
        this.setState({userAgent: userAgent, android: true, ios:false})
      }
    } else {
      this.setState({userAgent: navigator.userAgent, ios:false, android: false})
    }
  }

  render() {
    const config = this.state.userAgent ? {userAgent: this.state.userAgent} : {}
    return (
