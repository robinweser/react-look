...
describe('Sorting objects', () => {
  ...
  it('should return false if no object passed', () => {
    expect(sortObject('foo')).to.eql(false)
    expect(sortObject()).to.eql(false)
  })
  ...
  it('should sort nested objects', () => {
    const input = {a: 3, x: 5, b: {x: 12, b: 24}, d: 2}
    const output = {a: 3, b: {b: 24, x: 12}, d: 2, x: 5}
    expect(sortObject(input)).to.eql(output)
  })
})
